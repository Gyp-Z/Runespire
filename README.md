...
- **Hardcore countdown section (`#hardcore`)** — first section under the hero. Edge-blended red/purple **war-drum atmosphere** (no hard borders): an animated `::before` layer with a heartbeat (lub-dub) pulse over a purple undertone, fading into the hero above and About below. Intensity is **scroll-driven** via a `--heat` CSS var (`initHardcoreHeat` in main.js) that swells the red glow as the section fills the viewport. Contains the live countdown (DAYS/HOURS/MINUTES/SECONDS) to **2026-06-13 12:00 PM EST**, animated progress bar (June 1 → launch), pulsing seconds block, "THE LAST MAN STANDING" teaser card with skull watermark and a red Discord button. At zero, `js/launch-event.js` switches the site to launched mode (see Jarvis Sync).
...
3. **Hardcore SMP countdown + launch event system** (`#hardcore` section + `js/launch-event.js`) — crimson section under the hero counting down to **2026-06-13T12:00:00-04:00**. At zero (page open): timer swaps to a pulsing "⚔ THE HARDCORE SMP IS LIVE — JOIN NOW ⚔" banner, one-time rune flash, fire/ember canvas overlay ramps in, `body.launched` warms the palette and reddens nav/title glow, ambient particles gain fire tones. State persists in `localStorage('runespire_launched')`; post-launch visitors get everything except the rune flash. Reduced-motion users get the banner/palette only.
